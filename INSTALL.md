...
### 可选但推荐：应用 WSL PATH 清洗补丁

如果你在 `ao start`、orchestrator 日志，或者 agent 启动输出里看到 `/mnt/c/...`、`/mnt/d/...` 这类 Windows PATH 污染 warning，建议在 `Phase 3` 结束后立刻应用 kit 自带 patch，而不是等到后面排查时再回头补。

这里要特别注意兼容性边界：仓库里的
`patches/ao-main-2ebe111a-sanitize-windows-path.patch` 是按 2026-04-12 的
upstream `main` checkout 验证的，对应 AO 提交是
`2ebe111af002a19566dd03c96b7c9dbb817f000c`。它**不适用于**裸
`@composio/ao@0.2.2` 标签，因为那个版本里还没有
`packages/core/src/agent-workspace-hooks.ts` 这组文件。
...
- 当前提交哈希应与上面的兼容版本边界一致，或者至少保持在同一条较新的 upstream `main` 结构上。
- `test -f packages/core/src/agent-workspace-hooks.ts` 必须返回成功；如果这个文件不存在，说明你的 AO checkout 太旧，不要硬套这个 patch。
- 预检通过后，三条验证命令都应成功，后续 agent PATH 不再包含 `/mnt/<drive>/...` 这类条目。
...
前四条代理配置命令通常不会返回很多文字，但两条 `--get` 命令应分别打印形如下面这种值：
...
因此，这一步是**关键步骤**。它负责把你的 GitHub Personal Access Token 写入 `gh` 的认证存储，并让 Git 通过 GitHub CLI 的凭据接管步骤拿到可用凭据。
...
- 凭据接管步骤会提示 Git 已被配置为使用 GitHub CLI 作为凭据辅助程序。
...
1. `ao --help` 能运行。
2. `gh auth status` 正常。
3. 先取当前 `WSL_HOST_IP`，再执行一次远程读取检查，并确认它能返回结果。
...
最常见的原因不是网络，而是认证。请先回到 `Phase 7` 检查 `gh auth status` 和那一步凭据接管是否成功。其次，再确认你推送时是否像文档那样先取了当前 `WSL_HOST_IP`，并保留了 `HTTPS_PROXY="http://${WSL_HOST_IP}:7897"` 这个前缀。
